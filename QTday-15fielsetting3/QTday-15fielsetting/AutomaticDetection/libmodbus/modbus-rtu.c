...
//定义远程设备的从机ID，以便在主模式下对话，或设置从属模式下的内部从属ID
static int _modbus_set_slave(modbus_t *ctx, int slave)//写从机的设备地址 定义的静态全局变量
{
...
{
    assert(ctx->slave != -1);
    req[0] = ctx->slave;//从设备ID
    req[1] = function;
    req[2] = addr >> 8;//地址高位
    req[3] = addr & 0x00ff;//地址低位
    req[4] = nb >> 8;//数量高位
    req[5] = nb & 0x00ff;//数量低位
...
static uint16_t crc16(uint8_t *buffer, uint16_t buffer_length)//CRC校验算法程序
{
...
static ssize_t _modbus_rtu_send(modbus_t *ctx, const uint8_t *req, int req_length)//报文发送
{
#if defined(_WIN32)
...
static int _modbus_rtu_receive(modbus_t *ctx, uint8_t *req)//接受
{
    int rc;
    modbus_rtu_t *ctx_rtu = ctx->backend_data;
...
int modbus_rtu_set_serial_mode(modbus_t *ctx, int mode)//RTU串口模式
{
    if (ctx == NULL) {
        errno = EINVAL;
        return -1;
...
int modbus_rtu_get_serial_mode(modbus_t *ctx)//获取串口模式
{
    if (ctx == NULL) {
        errno = EINVAL;
        return -1;
...
int modbus_rtu_get_rts(modbus_t *ctx)//获取RTS模式
{
    if (ctx == NULL) {
        errno = EINVAL;
        return -1;
...
int modbus_rtu_get_rts_delay(modbus_t *ctx)//获取当前RTS延时
{
    if (ctx == NULL) {
        errno = EINVAL;
        return -1;
...
int modbus_rtu_set_rts_delay(modbus_t *ctx, int us)//设置RTS延时
{
    if (ctx == NULL || us < 0) {
        errno = EINVAL;
        return -1;
...
static void _modbus_rtu_close(modbus_t *ctx)//关闭连接
{
...
static int _modbus_rtu_flush(modbus_t *ctx)//冲洗为接收的数据
{
#if defined(_WIN32)
    modbus_rtu_t *ctx_rtu = ctx->backend_data;
...
const modbus_backend_t _modbus_rtu_backend = {
    ...
    _modbus_rtu_connect,//连接
    _modbus_rtu_close,//关闭modbus
    _modbus_rtu_flush,
    _modbus_rtu_select,
    _modbus_rtu_free//释放 modbus 资源
};
                         ...
                         int stop_bit)
{
    ...
    ctx_rtu->baud = baud;
    if (parity == 'N' || parity == 'E' || parity == 'O') {//奇偶性
